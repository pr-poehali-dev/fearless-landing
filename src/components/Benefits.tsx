const Benefits = () => {
  ...
  return (
    <section className="bg-black text-white py-20 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-96 h-96 bg-red-600 opacity-5 sharp-border"></div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="bg-gradient-to-br from-gray-900 to-black p-8 border-l-4 border-red-600 relative futuristic-shadow hover:scale-105 transition-all duration-300"
            >
              <div className="flex items-start gap-4 mb-6">
                <div className="text-4xl">{benefit.icon}</div>
                <div className="bg-red-600 text-white w-12 h-12 rounded-full flex items-center justify-center font-oswald text-xl font-bold diagonal-cut">
                  {benefit.number}
                </div>
              </div>

              <h3 className="font-oswald text-2xl font-bold mb-4 text-red-400">
                {benefit.title}
              </h3>
              ...
              {benefit.points && (
                <ul className="space-y-3">
                  {benefit.points.map((point, pointIndex) => (
                    <li
                      key={pointIndex}
                      className="flex items-start gap-3 font-open-sans text-gray-300"
                    >
                      <div className="w-2 h-2 bg-red-600 mt-2 diagonal-cut flex-shrink-0"></div>
                      {point}
                    </li>
                  ))}
                </ul>
